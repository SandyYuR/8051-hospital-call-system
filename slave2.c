/*****************************************************************************
 * 文件名：slave2.c
 * 项目  ：基于 AT89S51 单片机的医院病房呼叫系统 —— 从机 2（房间 2）
 * 功能  ：扫描 8 个按键（对应房间 2 的 8 张病床），按键按下后通过
 *         串口发送呼叫帧 0x20|床号（高 4 位为房间号 2，低 4 位为床号），
 *         收到主机回执 0x55 后，P2 口输出声光提示约 1 秒。
 * 通信  ：RS-485 总线（MAX485），波特率 9600。
 * 编译  ：Keil C51（AT89S51）
 * 说明  ：与 slave1.c / slave3.c 结构相同，仅房间号不同（0x10/0x20/0x30）；
