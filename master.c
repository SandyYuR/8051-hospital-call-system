/*****************************************************************************
 * 文件名：master.c
 * 项目  ：基于 AT89S51 单片机的医院病房呼叫系统 —— 主机程序
 * 功能  ：通过串口接收 1~3 号从机的呼叫信号，在 LCD12864 上显示
 *         "房间-床号"呼叫列表（最多 6 条），蜂鸣器声光报警，
 *         护士按下对应确认键后清除该条呼叫记录。
 * 通信  ：RS-485 总线（MAX485），波特率 9600，一字节数据帧：
 *         高 4 位 = 房间号，低 4 位 = 床号；
 *         主机收到数据后回发 0x55 作为接收确认。
 * 编译  ：Keil C51（AT89S51）
 * 说明  ：原始工程代码，仅补充注释，未改动任何逻辑。
 *****************************************************************************/
